...
from blobs_util2 import get_boxes_faster
...
def get_last_batch_mAPR(batch_truth, batch_out, threshold=0.25, heatmap_threshold=0.8, min_blob_size=None) :
    mAPs = []
    mARs = []
    for truth, out in zip(batch_truth, batch_out) :
        true_boxes = get_boxes_faster(truth.cpu().detach().numpy(), threshold=heatmap_threshold, min_blob_size=0)
        found_boxes = get_boxes_faster(out.cpu().detach().numpy(), threshold=heatmap_threshold, min_blob_size=0)
        # true_boxes = get_boxes(truth.cpu().detach().numpy(), threshold=0.5, min_blob_size=0)
        # found_boxes = get_boxes(out.cpu().detach().numpy(), threshold=heatmap_threshold, min_blob_size=min_blob_size)
